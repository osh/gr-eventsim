...
# Generated: Thu Nov 19 11:23:28 2015
##################################################
...
class test_sim_multi(gr.top_block):
    ...
    def __init__(self, nthreads=4, dps=500, l2=15.0, l1=10.0, ld=4.0):
        ...
        self.es_sink_1 = es.sink(1*[gr.sizeof_gr_complex],nthreads,256,0,2,"group1")
        self.es_sink_0 = es.sink(1*[gr.sizeof_gr_complex],nthreads,256,2,2,"group1")
        self.blocks_file_source_0 = blocks.file_source(gr.sizeof_gr_complex*1, "test.dat", True)

        ##################################################
        # Connections
        ##################################################
        self.msg_connect((self.es_trigger_sample_timer_0, 'which_stream'), (self.es_sink_1, 'schedule_event'))    
        self.msg_connect((self.es_trigger_sample_timer_0, 'sample_timer_event'), (self.eventsim_detector_0, 'handle_event'))    
        self.msg_connect((self.eventsim_detector_0, 'which_stream'), (self.es_sink_1, 'schedule_event'))    
        ...
        self.msg_connect((self.eventsim_operation2_0, 'which_stream'), (self.es_sink_1, 'schedule_event'))    
        ...
        self.connect((self.es_trigger_sample_timer_0, 0), (self.es_sink_1, 0))    
...
if __name__ == '__main__':
    ...
    parser.add_option("", "--nthreads", dest="nthreads", type="intx", default=4,
        help="Set nthreads [default=%default]")
    parser.add_option("", "--dps", dest="dps", type="eng_float", default=eng_notation.num_to_str(500),
        help="Set dps [default=%default]")
    parser.add_option("", "--l2", dest="l2", type="eng_float", default=eng_notation.num_to_str(15.0),
        help="Set l2 [default=%default]")
    parser.add_option("", "--l1", dest="l1", type="eng_float", default=eng_notation.num_to_str(10.0),
        help="Set l1 [default=%default]")
    parser.add_option("", "--ld", dest="ld", type="eng_float", default=eng_notation.num_to_str(4.0),
        help="Set ld [default=%default]")
    (options, args) = parser.parse_args()
    tb = test_sim_multi(nthreads=options.nthreads, dps=options.dps, l2=options.l2, l1=options.l1, ld=options.ld)
    tb.start()
    tb.wait()
